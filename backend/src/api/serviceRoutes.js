 ...
 * Sprint 4 / ERS:
 * - Solo servicios activos (isActive: true)
 * - Excluye servicios involucrados en trades COMPLETED
 * - Si hay token válido: excluye servicios del usuario actual (marketplace "otros")
 */
...
router.put("/:id", authenticate, async (req, res, next) => {
  try {
    ...
    const updatedService = await ServiceListingService.updateService(req.user.id, req.params.id, {
      title,
      description,
      category
    })
